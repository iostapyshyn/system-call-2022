...
int child_entry(void* arg) {
    ...
    if (arg) {
        ...
        int ret = setuid(0);
        syscall_write(": setuid()  = ", ret);  // What is the user id of this thread.
        syscall_write(": getuid()  = ", getuid());  // What is the user id of this thread.
    }
...
int main(int argc, char *argv[]) {
    ...
    if (!strcmp(argv[1], "fork")) {
        ...
        flags = CLONE_VM | CLONE_FILES | CLONE_FS |
            CLONE_IO | CLONE_THREAD | CLONE_SIGHAND;
    } else if (!strcmp(argv[1], "user")) {
        flags = SIGCHLD | CLONE_NEWUSER;

        arg = malloc(512*sizeof(char));
        snprintf(arg, 512, "0\t%d\t1\n", getuid());
    } else {
        printf("Invalid clone() mode: %s\n", argv[1]);
        return -1;
    }
    int ret = clone(child_entry, stack+sizeof(stack)-1, flags, arg);
    syscall_write("> clone() = ", ret);
    if (arg)
        free(arg);
